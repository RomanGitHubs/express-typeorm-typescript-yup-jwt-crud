...
const registerUserSchema = yup.object({
  body: yup.object({
    ...
    firstName: yup.string().matches(regexName, warningFirstName.matches).max(15, warningFirstName.max).required(warningFirstName.required),
    lastName: yup.string().matches(regexName, warningLastName.matches).max(15, warningLastName.max).required(warningLastName.required),
    email: yup.string().email(warningEmail.email).max(30, warningEmail.max).required(warningEmail.required),
    password: yup.string().matches(regexPassword, warningPassword.matches).min(6, warningPassword.min).required(warningPassword.required),
...
const editUserSchema = yup.object({
  body: yup.object({
    ...
    firstName: yup.string().matches(regexName, warningFirstName.matches).max(15, warningFirstName.max),
    ...
    password: yup.string().matches(regexPassword, warningPassword.matches).min(6, warningPassword.min),
    dob: yup.date(),
  })
});
